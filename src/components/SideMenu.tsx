...
export default function SideMenu() {
  ...
  return (
    <div>
      <div className={`flex md:hidden h-[30px] w-[30px]   justify-center items-center fixed top-[30px] right-[20px] z-30`} onClick={() => { setIsMeanOpen(!isMeanOpen) }}>
        <div className={`
    before:bg-blue-150 before:w-[100%] before:h-[5px] before:absolute before:top-0  before:transition-all before:duration-1000
     ...
     ${isMeanOpen ? "before:top-1/2 before:-translate-y-1/2 before:rotate-[135deg] after:bottom-1/2 after:translate-y-1/2 after:rotate-[45deg]" : ""}`}>
      ...
      <div onClick={() => { setIsMeanOpen(!isMeanOpen) }}
        className={`block md:hidden h-full w-full fixed top-0 right-0 backdrop-blur-sm  z-20 ${isMeanOpen ? " block" : " hidden"} `}></div>

      <div className={`h-full flex items-center justify-between bg-white fixed top-0 right-0 z-20 transition-[width]  md:hidden  duration-1000 ${isMeanOpen ? " w-[50vw]" : " w-0"}`}>
        <div className='h-[90%] p-6 '>
          <nav className='text-blue-150  h-[50%] flex flex-col  justify-between'>
            <ul className="h-full flex flex-col justify-evenly  text-2xl ">
